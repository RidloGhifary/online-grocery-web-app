...
export async function getJournals({
  ...
  const header: HeadersInit = {
    ...
    const response = await fetch(createQueryParams({
    ...
    const data = (await response.json()) as CommonPaginatedResultInterface<StockAdjustment[]>;
    result = data
...
export async function getStoreForStock() : Promise<CommonResultInterface<Store[]>> {
  ...
  try {
    ...
    const response = await fetch(`${process.env.BACKEND_URL}/stocks/store-list`,{headers:header})
